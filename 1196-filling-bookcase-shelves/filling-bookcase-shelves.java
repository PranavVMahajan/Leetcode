class Solution {
    ...
    public int findMinHeight(int bookIndex ,int[][] books,int curShelfWidth,int curShelfHeight,int dp[][]) {
        ...
        // case 2
        int newShelf = curShelfHeight+ findMinHeight(bookIndex+1,books,newShelfWeight-books[bookIndex][0],books[bookIndex][1],dp);
        return dp[bookIndex][curShelfWidth] = Math.min(sameShelf,newShelf);

    }
    public int minHeightShelves(int[][] books, int shelfWidth) {
        totalBooks = books.length;
        newShelfWeight = shelfWidth;
        int dp[][] = new int[totalBooks][newShelfWeight+1];
